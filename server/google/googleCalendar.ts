...
//Fetch and format calendar events for auser between a given date range
export async function getCalendarEventTimes(
  ...
  try {
    ...
    // Fetch events from the Google Calendar API code from the API
    const events = await google.calendar("v3").events.list({
      ...
      timeMin: eventDate.start.toISOString(), // Start of the time range (inclusive)
      timeMax: eventDate.end.toISOString(), // End of the time range (exclusive)
...
export async function createCalendarEvent(
  ...
  try {
    ...
    const oAuthClient = await getOAuthClient(calendarRegistration.clerkUserId);
    if (!oAuthClient) {
    ...
    const calendarUser = await client.users.getUser(
      calendarRegistration.clerkUserId,
    ...
    const calendarEvent = await google.calendar("v3").events.insert({
      ...
      requestBody: {
        ...
        description: calendarRegistration.guestNotes
          ? `Additional Details: ${calendarRegistration.guestNotes}`
